            ...
            <CardContent className="space-y-3">
              ...
              <div className="grid grid-cols-7 gap-1.5">
                {Array.from({ length: calendar[0]?.weekday ?? 0 }).map((_, index) => (
                  <div key={`pad-${index}`} className="aspect-square rounded-xl border border-transparent" />
                ))}
                {calendar.map((day) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "flex aspect-square flex-col rounded-xl border px-2 py-2 text-left transition-colors sm:px-3 sm:py-3",
                        isSelected ? "border-primary/40 bg-primary/10" : "border-border bg-muted/25 hover:bg-accent/70"
                      )}
                    >
            ...
            <CardContent className="space-y-3">
              {filteredSessions.length === 0 ? (
                ...
                filteredSessions.map((session) => (
                  <div key={session.id} className="list-item-row-stacked">
