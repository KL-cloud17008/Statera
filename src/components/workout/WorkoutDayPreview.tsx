  ...
  return (
    <div>
      ...
      <div className="mt-5 flex flex-wrap items-center justify-between gap-4 border-b border-rule pb-5">
        <p className="text-body text-secondary"><span className="font-medium tabular-nums text-primary">{exercises.length}</span> exercises <span aria-hidden className="px-2">·</span><span className="font-medium tabular-nums text-primary">{totalSets}</span> working sets</p>
        ...
        <div className="divide-y divide-rule">
          {exercises.map((exercise, index) => <div key={exercise.id} className="grid grid-cols-[2rem_minmax(0,1fr)] gap-3 py-4 sm:grid-cols-[2rem_minmax(0,1fr)_auto]">
            ...
            <div>
              ...
              <p className="mt-1 text-caption text-secondary">{exercise.sets} × {exercise.reps}{exercise.targetRPE ? ` · RPE ${exercise.targetRPE}` : ""}{exercise.tempo ? ` · Tempo ${exercise.tempo}` : ""}</p>
